...
- 数据就绪：正常执行 I/O，返回结果
- 数据未就绪：立即返回（`errno = EWOULDBLOCK` 或 `EAGAIN`），线程不阻塞
...
- 缺点 3：内核采用轮询方式检查 fd 状态，时间复杂度 O (n) --> 遍历传入的 所有 fd，检查是否有就绪事件
- 适用场景：跨平台小程序，fd 数量少
...
>
> * <u>**`sleep()` 期间，对应的进程 / 线程不会占用 CPU 资源，CPU 会被调度去执行其他就绪的进程 / 线程**。</u>
>   * <u>`sleep()`的核心作用是：**让当前进程（或线程）主动放弃 CPU 使用权，进入「休眠 / 阻塞状态」，并在指定的时间后被系统唤醒，重新进入「就绪状态」等待 CPU 调度**。</u>
>
...
> ###### 🔥 select的特点
>
> 1. **当 `select()` 没有就绪 fd 时，当前进程会被内核挂起（进入阻塞状态），既不占用用户态 CPU 资源，也不会持续占用内核态 CPU 资源，CPU 会被调度去执行其他就绪进程 / 线程**。
> 2. 内核采用的是「**事件驱动**」模式，而不是「轮询」模式。
>    * 轮询模式（比如用户态的忙等待）会持续占用 CPU，因为需要不断循环判断；而事件驱动模式是「有事件才响应」，没有事件时内核不做任何关于该 fd 的操作，不会消耗 CPU 资源。
>    * 只有当硬件 / 驱动触发中断（比如网卡收到数据、键盘输入字符）时，内核才会处理对应的 fd 就绪事件，唤醒挂起的进程。
...
> * `select`：维护三个文件描述符集合 fd_set，支持可读、可写、异常
>
>   * 核心函数：`int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout);`
>   * 缺点：
>     * fd集合是输入输出参数，每次调用都需要**重新初始化**：因为输入的fd集合在调用select后内核只会返回就绪的fd，修改了原来的fd集合
>     * 而且由于仅返回了就绪的总fd数，需逐个 FD 判断 FD_ISSET，在三个集合中是否存在（用`&`位与判断），即**遍历**所有fd集合
>     * 监控的fd数量有宏定义**上限**1024
>
> * `poll`：维护`pollfd`事件结构体数组
>
>   * 核心函数：`int poll(struct pollfd *fds, nfds_t nfds, int timeout);`就这一个函数少👍
>   * pollfd事件结构体封装了fd、监控事件和实际发生事件，实现了监控和返回事件的分离-->因此不需要再每次调用poll函数时重新初始化pollfd事件结构体；内核会修改传入pollfd结构体的revents，即发生的事件，因此只需要遍历所有revents（也是用位与判断）就可以知道哪些fd准备就绪；所有待监控事件存储在events，即要监控事件中，通过位或保存
>   * 缺点：
>     * 和select一样每次调用poll函数都需要拷贝pollfd结构体数组（不需要初始化，但是假设在动态数组中加入新连接事件，那么下一次就需要**重新拷贝**结构体数组到函数）
>     * 尽管poll函数返回就绪的fd数量，仍需**遍历**pollfd数组的revents，判断就绪事件是什么
>
> * `epoll`：维护`epoll_event`事件
>
>   * 核心函数：
>
>     * `int epoll_ctl(int epfd, int op, int fd, struct epoll_event *event); // 返回值：0 / -1 + errno`
>     * `int epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout);`
>
>   * epoll_event事件封装了事件类型和自定义联合体结构体data，每次通过epoll_ctl函数**管理** 需要监控的fd和对应事件；
>
>   * 在data中存储对应fd的相关信息（可以是fd）-->  <u>就绪事件本身就携带了识别信息，解耦了操作和回调</u>，每一个epoll_event事件包含了事件和对应的句柄，epoll_ctl函数的fd表明了对哪一个fd进行管理，event.data.fd是「事件回调时的附加数据」**当 `epoll_wait()` 检测到该 fd 就绪时，把这个值原封不动地返回给用户进程，方便用户进程识别「哪个 fd 就绪了」，并进行后续处理**。
>
>   * epoll_wait返回的是就绪的fd数量，通过` struct epoll_event events[1024]; // 保存就绪事件`配合返回值n，遍历0～n就是所有已经就绪的fd，无需再判断fd是否就绪-->直接遍历就绪事件数组，实现了**事件驱动遍历**
>
>     > 注意⚠️：需要一个缓存区存放eoll_wait返回的epoll_event事件

## 关键问题 💥

> Q： 为什么pollO (n)（遍历全部监控 fd）epollO (1)（仅遍历就绪 fd，无就绪则不遍历）？
>
> poll每次调用拷贝全部 pollfd 数组到内核，返回时拷贝就绪状态到用户态，这样需要遍历pollfd的revent判断是否就绪和就绪事件是什么，对吗？epoll仅初始化时拷贝监控 fd 到内核，后续通过 epoll_ctl 增删，就绪事件仅拷贝 “就绪 fd” 到用户态，epoll_wait返回就绪fd数量，通过struct epoll_event *events存储返回的就绪fd epoll事件，遍历这些事件不需要判断是否就绪，只需要判断是什么事件就可以了？ 所以么pollO (n)（遍历全部监控 fd）epollO (1)（仅遍历就绪 fd，无就绪则不遍历）是因为poll需要判断pollfd对应的revent是否是就绪的？
>
> A：poll在内核遍历pollfd填充revents，（就绪的填对应事件，未就绪的填 0），然后把全量 `pollfd` 数组拷贝回用户态。用户态需要额外判断fd是否就绪
>
> 1. **`poll O(n)` 的核心原因**：每次调用 `poll()` 都要拷贝全量 `pollfd` 数组到内核，返回后**用户态必须遍历全部监控的 `fd`（即全量 `pollfd` 数组），通过检查 `revents` 是否非空（或匹配目标事件）来判断是否就绪**，这个「全量遍历」就是 `O(n)` 的来源（`n` 是监控的 `fd` 总数）。
> 2. **`epoll O(1)` 的核心原因**：仅初始化（`epoll_ctl` 添加）时拷贝 `fd` 到内核并持久化存储，后续 `epoll_wait()` 仅拷贝「就绪 `fd`」的事件到用户态，返回后**用户态只需遍历「就绪 `fd` 数量」的元素（而非全量），且这些元素都是已就绪的，无需判断「是否就绪」，只需判断「是什么事件」**，这个「仅遍历就绪 `fd`」的逻辑，加上内核的高效就绪事件组织，构成了 `epoll` 接近 `O(1)` 的高效性。
